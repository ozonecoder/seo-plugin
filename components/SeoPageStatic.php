<?php namespace Ozc\SEO\Components;

use App;
use URL;
use Request;
use Cms\Classes\Page;
use Cms\Classes\Theme;
use Ozc\SEO\models\Settings;
use Cms\Classes\ComponentBase;
use RainLab\Pages\Classes\Router;

class SeoPageStatic extends ComponentBase{
	public $page;
	public $seo_title;
	public $seo_description;
	public $seo_keywords;
	public $canonical_url;
	public $redirect_url;
	public $robot_index;
	public $robot_follow;
	public $hasBlog;

	public $ogTitle;
	public $ogUrl;
	public $ogDescription;
	public $ogSiteName;
	public $ogFbAppId;
	public $ogLocale;
	public $ogImage;

	public $settings;

	public function componentDetails(){
		return [
			'name'        => 'SEO Page Static',
			'description' => 'Inject SEO Fields of Static pages'
		];
	}

	public function defineProperties(){
		return [];
	}

	public function onRun(){
        $url = Request::path();

        // Remove language prefix in case it exists (e.g. from "/en/my-page" to "/my-page")
//        if (class_exists('RainLab\Translate\Behaviors\TranslatableModel'))
//            $url = substr($url, 0);

        if (!strlen($url))
            $url = '/';

        $router = new Router(Theme::getActiveTheme());
        $this->page = $this->page['page'] = $router->findByUrl($url);
        $this->settings = $this->page["settings"] = Settings::instance();


        if ($this->page) {
            $this->seo_title = $this->page['seo_title'] = empty($this->page->getViewBag()->property('seo_title')) ? $this->page->title :  $this->page->getViewBag()->property('seo_title');
            $this->seo_description = $this->page['seo_description'] = $this->page->getViewBag()->property('meta_description');
            $this->seo_keywords = $this->page['seo_keywords'] = $this->page->getViewBag()->property('seo_keywords');
            $this->canonical_url = $this->page['canonical_url'] = $this->page->getViewBag()->property('canonical_url');
            $this->redirect_url = $this->page['redirect_url'] = $this->page->getViewBag()->property('redirect_url');
            $this->robot_follow = $this->page['robot_follow'] = $this->page->getViewBag()->property('robot_follow');
            $this->robot_index = $this->page['robot_index'] = $this->page->getViewBag()->property('robot_index');

            $settings = $this->settings;
            if($settings->enable_og_tags)
            {
                $this->ogTitle = empty($this->page->getViewBag()->property('seo_title'))
                    ? $this->page->title : $this->page->getViewBag()->property('seo_title');
                $this->ogDescription = $this->page->getViewBag()->property('meta_description');
                $this->ogUrl = empty($this->page->getViewBag()->property('canonical_url')) ? Request::url() : $this->page->getViewBag()->property('canonical_url');
                $this->ogSiteName = $settings->og_sitename;
                $this->ogImage = $this->page->image;
                $this->ogFbAppId = $settings->og_fb_appid;

            }
        }

		//prevent redirects in dev or stating
		if(App::environment() != 'production'){
			$this->redirect_url = null;
		}


	}

}